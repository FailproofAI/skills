...
dashboard-only, which `agenteye` reaches, and which only the HTTP API reaches.** Those three
sets differ, and none contains the others.

Anchors are into the docs tree (`docs/sessions/*.mdx`, `docs/reference/*.mdx`,
`docs/reference/openapi.json`) and the shipped `agenteye-cli` / `agenteye-evaluator` skills.
This file writes `agenteye`; the docs write `fp` — same tool, mid-rename. To actually run
queries hand off to **`agenteye-cli`**: it owns flags, pagination and exit codes in depth.
...
| CLI | `agenteye events …` | `agenteye events --full …` |
...
- **`--search` scans payloads but never returns them** — you can find an event by its text
  ...
  (`agenteye-cli/references/commands.md`, grep `responses remain payload-free`).
...
| Step | Run | Watch for |
|---|---|---|
| Goal and environment | `agenteye --json sessions --session-id <id>`, plus `agent_start`'s `goal` | labels are set by the emitting SDK/daemon and are **not retroactive** — a relabel splits history across two filter values |
| **First divergence, not last error** | `agenteye events --session-id <id> --order asc --all --limit 1000` | the light feed's per-event summaries are enough; never omit the explicit limit |
| Model context and tool input just before it | `agenteye --json events --full --session-id <id> --all --limit 1000`, or `query run --sql "SELECT id, event_type, payload FROM events WHERE session_id='<id>' ORDER BY ts"` | an empty payload may be capture being off, not silence |
...
Never reconstruct a session's shape from `agenteye sessions`. That feed is anchored on
...
The most useful table here. "CLI" is `agenteye`; "API" is the public `/v1` surface on your
...
| Block rate, Cloud decisions, unguarded actions, machine filter, policy mapping | Observe → policy | **no** | **no** — deployment is deliberately outside `/v1` |
| Failproof Assistant | Open agent chat | `agent …` | **no endpoint of any kind** |

Two rows invert the usual assumption. **Dashboards are not UI-only:** `dashboards.mdx` says
...
Neither has a dedicated CLI command; the documented approximation for denials is `agenteye
events --search "deny" --since 24h`, which matches any payload containing that string —
brittle, and say so when you use it. For the machine's own enforcement state use
`failproofai config --status`, not the cloud CLI.
...
- **`metrics.mdx` sends you to the wrong key list.** It says run `agenteye list score_filters`
...
`dashboards`, `saved_queries`, `api_keys`, `users` (operational). Run `agenteye query
...
- Two docs conflicts, both read off the pages and **not tested against a running build**:
  ...
  `agenteye query run --help`, use `query list --show-id`, and prefer the name.
...
`agenteye agent health` → `agent models` → `agent ask "…"` (prints a short chat id) → `agent
...
|---|---|
| `agenteye events --since 24h --all` | `--all` is bounded by `--limit`, default **50**. You get 50 rows and `next_cursor: null` — it looks complete. Pass `--all --limit 1000`, and cross-check with `--aggregate`, which covers the whole window regardless of row caps |
| `agenteye events --json` | Global flags go **before** the command; after it they are a usage error, exit 2. Write `agenteye --json events` |
...
| `agenteye evals --env prod --env staging` | `evals` and `errors` filters are **single-valued** — last wins, so this queries only staging. `sessions` and `events` filters do accumulate |
...
| `agenteye whoami` to test login | It **never exits 4**; it returns `{"logged_in": false, …}` and exit 0. Branch on the field, not the code |
...
| API-key mode targeting the wrong tenant | Saved human-session org state is **intentionally ignored** for API-key requests. Pass `--org <slug>`, `AGENTEYE_ORG`, or the `X-AgentEye-Org` header |
