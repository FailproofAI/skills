...
can replay through your evaluator. Both go through the `fp` CLI, which
needs a logged-in session (`fp login`) and a dashboard URL
...
(`fp --json orgs current` tells you the slug). If it 401s, the session
expired: `fp login`. If it 403s, the login lacks `events:read`.
