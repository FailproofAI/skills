 ...
 *
 * `--cli <name>` runs the hook as that agent CLI and, before running anything,
 * checks the (cli, event) pair against references/harnesses.md. A green PASS
 * only proves the policy *decided* deny — on many (cli, event) pairs the host
 * discards that verdict, so the tool still runs. Those pairs are reported as
 * INERT rather than PASS, because "the test passed" is exactly the sentence
 * that lets an unenforced policy ship.
 */
...
Usage:
  ...
  --cli      Agent CLI to run as: claude (default), codex, copilot, cursor,
             opencode, pi, hermes, openclaw, factory, devin, antigravity, goose.
             Also checks whether a deny on this event actually enforces there.
...
A case may set its own "cli" to run the same rule against several harnesses.
`.trim());
...
/**
 * Whether a deny on this (cli, event) pair actually stops anything.
 *
 * Generated by sync-harnesses.mjs from failproofai's ENFORCEMENT_CAPABILITY.
 * Missing file or missing row is reported as unknown, never as "block" — the
 * source table's own rule is that absent means unverified, and asserting
 * protection nobody traced is the failure this whole check exists to prevent.
 */
const CAPABILITY = (() => {
  const p = join(HERE, "harness-capability.json");
  if (!existsSync(p)) return null;
  try {
    return JSON.parse(readFileSync(p, "utf8")).events ?? null;
  } catch {
    return null;
  }
})();

function capabilityFor(cli, event) {
  if (!CAPABILITY) return null;
  return CAPABILITY[event]?.[cli] ?? null;
}
...
function runCase(c, sandbox) {
  ...
  if (cli && cli !== true) hookArgs.push("--cli", cli);

  const res = spawnSync(runner.cmd, [...runner.pre, ...hookArgs], {
...
try {
  for (const c of cases) {
    const cli = c.cli ?? (typeof args.cli === "string" ? args.cli : "claude");
    const got = runCase(c, sandbox);
    const label = c.name ?? `${c.event} ${c.tool ?? ""}`.trim();
    const where = cli === "claude" ? "" : `  [${cli}]`;
    if (!c.expect) {
    ...
    // A policy can decide correctly and still enforce nothing: on an `observe`
    // pair the host runs the hook, reads the deny, and proceeds anyway. That is
    // a PASS by every assertion in this file, which is precisely why it gets
    // its own label — reporting it as a plain PASS is how an unenforced policy
    // ships with a green suite behind it.
    const cap = c.expect === "deny" ? capabilityFor(cli, c.event) : null;
    const flag =
      cap === "observe" ? "  INERT" : cap === "not-installed" ? "  NOT-INSTALLED" : cap === "unverified" ? "  UNVERIFIED" : "";
    if (ok && flag) inert.push({ label, cli, event: c.event, cap });

    console.log(
      `  ${ok ? "PASS" : "FAIL"}  ${label}${where}  (expected ${c.expect}, got ${got.decision})${flag}`,
    );
...
if (inert.length) {
  console.log("\nThe deny was produced, but the harness does not act on it:");
  for (const i of inert) {
    const why =
      i.cap === "observe"
        ? `${i.cli} runs ${i.event} but discards the verdict — the action still happens`
        : i.cap === "not-installed"
          ? `failproofai does not install ${i.event} for ${i.cli} — the policy never runs there`
          : `${i.cli}/${i.event} is not traced in enforcement-capability.ts — enforcement unproven`;
    console.log(`  • ${i.label}: ${why}`);
  }
  console.log("  See references/harnesses.md for where to move the rule.");
}
